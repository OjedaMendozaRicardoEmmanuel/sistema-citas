...
import { Expediente } from 'src/app/services/models/expediente';
...
export class ExpedientesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const getExpedientes$ = this.apiService.getAllExpedientes();
    ...
    forkJoin([
      ...
      getUsuarios$,
      getExpedientes$
    ]).subscribe(([citas, pacientes, odontologos, usuarios, expedientes]) => {
  ...
  verHistorial(px: Paciente) {
    ...
    this.expedientesPX = this.obtenerExpedientes();
  ...
  obtenerExpedientes():Expediente[]{
    let exp:Expediente[] = [];
    this.citasPX.forEach(ct => {
      this.expedientes.forEach(res=>{
        if (ct.id == res.cita_id) {
          exp.push(res);
        }
      });
    });
